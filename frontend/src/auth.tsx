...
function ActivationPage({token,onLogin,currentUser,onLogout}:{token:string;onLogin:(user:ApiUser)=>void;currentUser:ApiUser|null;onLogout:()=>void}){
  ...
  return <main className="login-screen"><section className="login-card activation-card"><BrandHeader/>{loading?<div className="auth-loading">正在验证邀请链接…</div>:!invitation?<><div className="login-intro"><h1>邀请无法使用</h1><p>{error||"邀请链接不存在、已过期或已经被使用。"}</p></div><button className="auth-secondary" onClick={()=>navigate("/",{replace:true})}>返回登录</button></>:<><div className="login-intro"><span className="activation-badge"><UserPlus size={15}/> 团队邀请</span><h1>{currentUser?"确认邀请账号":"激活你的账号"}</h1><p>{invitation.inviter_name} 邀请你加入「{invitation.team_name}」{invitation.project_name?`，并参与项目「${invitation.project_name}」`:""}。</p></div><div className="invitation-summary"><CheckCircle2 size={17}/><span><strong>{invitation.email}</strong><small>角色：{invitation.role==="CEO"?"团队管理员":"团队成员"} · 邀请将在 {new Date(invitation.expires_at).toLocaleString("zh-CN")} 失效</small></span></div>{currentUser?<div className="active-session-warning"><div><strong>当前已登录</strong><span>{currentUser.name} · {currentUser.email}</span><small>{sameAccount?"该邮箱已经拥有账号，不能再次通过新账号邀请激活。请返回当前账号，或联系管理员重新处理成员权限。":`此邀请发送给 ${invitation.email}，不能使用当前账号直接激活。请先退出当前账号，再继续激活受邀账号。`}</small></div><div className="active-session-actions"><button className="auth-secondary" onClick={()=>navigate("/",{replace:true})}>返回当前账号</button>{!sameAccount&&<button className="auth-switch-account" onClick={onLogout}>退出并继续</button>}</div></div>:<><form onSubmit={submit}><label><span>显示名称</span><input autoFocus value={name} onChange={event=>setName(event.target.value)} placeholder="你的真实姓名" minLength={2} maxLength={80}/></label><label><span>设置密码</span><input type="password" autoComplete="new-password" value={password} onChange={event=>setPassword(event.target.value)} placeholder="至少 10 位，同时包含字母和数字" minLength={10}/></label><label><span>确认密码</span><input type="password" autoComplete="new-password" value={confirmation} onChange={event=>setConfirmation(event.target.value)} placeholder="再次输入密码" minLength={10}/></label>{error&&<p className="login-error">{error}</p>}<button disabled={submitting||name.trim().length<2||password.length<10||!confirmation}>{submitting?"正在激活…":<><UserPlus size={16}/> 激活并进入团队</>}</button></form><footer><ShieldCheck size={15}/> 邀请只能使用一次；激活后权限仍受团队和项目范围控制。</footer></>}</>}</section></main>;
}
