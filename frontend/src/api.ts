...
async function request<T>(path:string, options:RequestInit={}):Promise<T> {
  ...
  if(!response.ok){let message=`请求失败（${response.status}）`;try{const body=await response.json();message=body.detail||message;}catch{/* status fallback */}throw new ApiError(response.status,message);}
