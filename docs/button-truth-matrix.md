...
| 我的贡献 | READ_ONLY | 只读取 `/api/contributions` 的验收事件；求助、知识、开放任务、趋势和徽章未接入时显示“— / 未接入” |
| 项目风险与依赖 | READ_ONLY | 仅由项目真实阻塞任务和等待外部任务派生，不展示固定风险 |
| 项目会议、项目资产写入 | DISABLED | 尚无持久化和权限接口，不展示固定会议或本地资产 |
| Agent 接入 | DISABLED | 尚无稳定的外部凭证、CLI 和公开 API 契约，仅展示能力边界 |
