...
项目级 AI 拆解与任务总览已接入现有闭环：项目成员可基于项目真实目标、阶段、任务和成员生成候选小任务，在单项目页面集中修改阶段、负责人、验收人、截止时间与人工/AI/人机协同方式；人工确认后才创建正式任务。`ai-task` 薄 CLI 复用同一 API，详见 `docs/project-ai-decomposition-cli-plan.md` 和 `docs/cli.md`。
