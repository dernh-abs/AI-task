...
def project_reads(session: Session, project_ids: list[str]) -> list[ProjectRead]:
    ...
    for project in projects:
        ...
        results.append(ProjectRead(id=project.id, name=project.name, client=project.client, objective=project.objective, owner_id=project.owner_id, owner_name=names.get(project.owner_id, "未指定"), next_milestone=project.next_milestone, due_at=project.due_at, progress=progress, health=health, health_reasons=health_reasons, current_stage=current_stage, stages=stage_reads))
    return results
