...
def seed_demo_data() -> None:
    with Session(engine) as session:
        ...
        # SQLModel records below intentionally do not define ORM relationships.
        # PostgreSQL therefore needs explicit dependency phases; SQLite's lax
        # foreign-key defaults previously hid this ordering requirement.
        phases = [
            [member, ceo, observer, team],
            [
                ExternalContact(id="contact-client", team_id=team.id, name="客户联系人", organization="客户方", channel="企业微信"),
                TeamMember(team_id=team.id, user_id=member.id, role=TeamRole.MEMBER),
                TeamMember(team_id=team.id, user_id=ceo.id, role=TeamRole.CEO),
                TeamMember(team_id=team.id, user_id=observer.id, role=TeamRole.MEMBER),
                project,
            ],
            [
                ProjectMember(project_id=project.id, user_id=member.id, role=ProjectRole.MEMBER),
                ProjectMember(project_id=project.id, user_id=ceo.id, role=ProjectRole.OWNER),
                stage,
            ],
            tasks,
        ]
        for records in phases:
            for record in records:
                identity = record.__mapper__.primary_key_from_instance(record)
                identity_key = identity[0] if len(identity) == 1 else identity
                if session.get(type(record), identity_key) is None:
                    session.add(record)
            session.commit()
